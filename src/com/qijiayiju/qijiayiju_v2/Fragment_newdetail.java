...
public class Fragment_newdetail extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		// TODO Auto-generated method stub
		View view = inflater.inflate(R.layout.fragment_new_detail, container,
				false);
		listView_palce = (ListView) view.findViewById(R.id.listView1);
		button_detail2 = (Button) view.findViewById(R.id.button_detail2);
		textView_detail_wendu = (TextView) view
				.findViewById(R.id.textView_detail_wendu);
		textView_detail_shidu = (TextView) view
				.findViewById(R.id.textView_detail_shidu);
		dbUtils = DBHelper.getUtils();
		// Cursor
		// cur=dbUtils.execQuery("select * from Data where id=(select max(id) from Data)");
		// int nameColumn = cur.getColumnIndex("id");
		// String name = cur.getString(nameColumn);
		// String string = cur.getString(0);
		// Data data2
		// =dbUtils.findFirst(Selector.from(Data.class).where("id","=",
		// "(select max(id) from Data)"));
		// List<Data> list = DBHelper.getUtils().findAll(Data.class);//通过类型查找
		// Data data=list.get(list.size()-1);
		textView_detail_shidu.setText((float) (Math
				.round(MainActivity.shiwaishidu * 10)) / 10 + "Rh%");
		textView_detail_wendu.setText((float) (Math
				.round(MainActivity.shiwaiwendu * 10)) / 10 + "℃");

		rooms = getResources().getStringArray(R.array.room);
		adapter = new ArrayAdapter<String>(getActivity(),
				R.layout.listview_item, rooms);
		...
		listView_palce.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if (arg2 == 1) {
					textView_detail_shidu.setText((float) (Math
							.round(MainActivity.shineishidu * 10)) / 10 + "Rh%");
					textView_detail_wendu.setText((float) (Math
