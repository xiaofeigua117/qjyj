...
private void setReminder(boolean b,int hour,int min) {
        ...
        PendingIntent pi= PendingIntent.getBroadcast(getActivity(), 0, new Intent(getActivity(),MyReceiver.class), 0);
...
//        	 futureDate.setSeconds(0);
		...
		button.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				try {
					...
					//list = dbUtils.findAll(Selector.from(Data.class).where("id" ,"<", 800));
					list = dbUtils.findAll(Selector.from(Data.class));  
		           // dbUtils.delete(stus.get(0));  
		            dbUtils.deleteAll(list);  
		            Toast.makeText(getActivity(), "删除完毕", Toast.LENGTH_LONG).show();
		           // dbUtils.deleteById(Data.class, WhereBuilder.b("date", "==", "02 10 00 81 00 01 51 D2"));  
		  ...
		  button_tongfeng.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				new AlertDialog.Builder(getActivity()).setTitle("请选择开启时的通风时间").setIcon(
					     android.R.drawable.ic_dialog_info).setPositiveButton("确定", null).setSingleChoiceItems(new String[]{"10分钟","20分钟","40分钟"}, 0, new DialogInterface.OnClickListener() {
	...
	button_shijian.setOnClickListener(new OnClickListener() {
		...
		button_naozhong.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				// TODO Auto-generated method stub、
				Intent intent1=new Intent(getActivity(),TongfengAutoActivity.class);
		...
		public void showDialog(){
			...
			TimePickerDialog pickerDialog=new TimePickerDialog(context, onTimeSetListener, hour, min, true);
			pickerDialog.show();
